...
def get_dataset(app_launch_log,user_activity_log,user_register_log,video_create_log):
    ...
    off_tr = pd.DataFrame()
    i = 0
    print(str(round(i/7*100)) + '%...',end = '')
    i += 1
    for start in [23,22,21,20,19]:
        # 获得该start前注册的所有用户
        ...
        # 提特征
        feat_dates = list(range(start - 18,start))
...
#    ## 保存
#    pickle.dump(off_tr,open('tmp/off_train_1.pkl','wb'),protocol = 4)
#    pickle.dump(va,open('tmp/off_validate_1.pkl','wb'),protocol = 4)
#    pickle.dump(on_tr,open('tmp/on_train_1.pkl','wb'),protocol = 4)
#    pickle.dump(te,open('tmp/on_test_1.pkl','wb'),protocol = 4)
